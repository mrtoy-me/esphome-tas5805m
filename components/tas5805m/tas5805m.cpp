...
namespace esphome::tas5805m {
        ...
        while (i < number_configurations) {
            switch (TAS5805M_REGISTERS[i].offset) {
                case TAS5805M_CFG_META_DELAY:
                    ...
                    break;
                default:
                    if (!this->tas5805m_write_byte_(TAS5805M_REGISTERS[i].offset, TAS5805M_REGISTERS[i].value))
                        return
                                false;
                    counter++;
                    break;
    ...
    this->publish_faults_();
        ...
        (this->tas5805m_faults_.is_fault_except_clock_fault || (
             this->tas5805m_faults_.clock_fault && (!this->ignore_clock_faults_when_clearing_faults_)));


        // if no change in faults bypass publishing
        if (!(this->is_new_common_fault_ || this->is_new_over_temperature_issue_ || this->is_new_channel_fault_ || this
              ->is_new_global_fault_))
            return;
  ...
  *state = this->tas5805m_control_state_;
  return true;
    ...
    bool Tas5805mComponent::read_fault_registers_() {
        ...
        this->is_new_over_temperature_issue_ = (this->is_new_over_temperature_issue_ || (
                                                    current_faults[3] != this->tas5805m_faults_.temperature_warning));
  ...
  this->tas5805m_faults_.have_fault = new_fault_state;
    ...
    bool Tas5805mComponent::tas5805m_paged_write(uint8_t book, uint8_t page, uint8_t a_register, uint8_t *data,
                                                 uint8_t len) {
